...
//        elseif (is_paged() && $class_name = $this->get_paged_class_name()):
  ...
  public function get_taxonomy_class_name() {
    ...
    if ( ! empty( $term->slug ) ) {
      ...
      $slug_array    = explode( '-', $taxonomy );
      $template_slug = array_map( function ( $val ) {
        return ucfirst( $val );
      }, $slug_array ); // @todo preg_split()

      $taxonomy = implode( '_', $template_slug );
  ...
  public function get_page_class_name() {
    $template_slug = get_page_template_slug();

    if ( is_page_template( $template_slug ) ) {
      $slug_array    = explode( '-', str_replace( '.php', '', $template_slug ) );
      $template_slug = array_map( function ( $val ) {
        return ucfirst( $val );
      }, $slug_array ); // @todo preg_split()

      $template_slug = implode( '_', $template_slug );
    }


    $class_names[] = "Page_{$template_slug}";
    $class_names[] = "Page";

    return $this->get_query_class_name( 'page', $class_names );
  }
  ...
  public function locate_class_name( array $class_names ) {
    ...
    foreach ( (array) $class_names as $class_name ) {
      $class_name = $this->add_affix( $class_name );
